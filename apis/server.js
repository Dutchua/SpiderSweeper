...
const config = {
  ...
  server:
    "terraform-20240507114400697600000001.cex3uty77nu9.eu-west-1.rds.amazonaws.com",
  port: 1433,
...
//
app.get("/sign-in", cors(corsOptions), async (req, res) => {
  ...
  try {
    let pool = await sql.connect(config);
    const query = `IF NOT EXISTS (select * from users where username = '${username}')
                BEGIN
...
app.post("/new-score", cors(corsOptions), async (req, res) => {
